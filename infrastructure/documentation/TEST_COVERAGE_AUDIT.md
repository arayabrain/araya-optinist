...
| -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PremiumHeartbeatRetry.test.ts`                                      | Imports nothing but `@jest/globals`. Re-declares `HEARTBEAT_MAX_RETRIES = 3` and re-implements the retry loop inside the test, then asserts against its own copies. Deleting the real retry loop leaves it green |
