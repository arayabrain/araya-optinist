...
| Dataview (`06-dataview`)                     | DV-01..16 | table display, ID/name column-menu filters, sort, pagination, inputs/outputs/details dialogs, public access, public/private API auth, image/ROI thumbnails, publish/unpublish + public listing, bulk publish/unpublish with confirmation                                                                                                                                                            | DB/S3 sync verification (the sync-status UI is covered by `SyncStatusView.test.tsx`) |
| Subscription (`07-subscription`)             | SUB-01..14 | free and premium plan UI state, `/thanks` access guard, invoice page sections and rows, cancel and reactivate, the checkout and billing-portal hand-offs | Stripe-hosted checkout and portal pages, DB/Stripe dashboard verification |
| Storage (`08-storage`)                       | STO-01..03 | no-warning login under quota, dedicated and shared premium assignment on login | S3 verification, over-quota states (see the lifecycle group), auto-refresh tracing |
...
| BT-718    | Pending Sync Status Display            | `ImagePlotSimple.test.tsx` (sync indicator while loading and not yet initialized)                                                                 |
| BT-719    | Manual Retry from Sync Error           | `ImagePlotSimple.test.tsx` / `RoiPlotSimple.test.tsx` (retry button appears on a syncing error and refetches); the S3 re-sync itself stays manual |
