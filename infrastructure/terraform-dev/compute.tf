...
resource "aws_lb" "autoscaling" {
  ...
  tags = {
    Name = "development-optinist-load-balancer"
  }
}

# Load Balancer Listener - HTTP (test env: no HTTPS/custom domain)
...
# Alias for resources that reference autoscaling_https (premium_manager, etc.)
# In test env this is the same HTTP listener on port 80
...
resource "aws_lb_target_group" "autoscaling" {
  ...
  tags = {
    Name = "development-optinist-cloud-target-group"
  }
}
...
resource "aws_launch_template" "ecs" {
  name_prefix   = "development-optinist-ecs-"
  image_id      = data.aws_ami.ecs_optimized.id
  instance_type = "t3.large"
  key_name      = aws_key_pair.development_optinist_cloud_key_pair.key_name
  ...
  tag_specifications {
    ...
    tags = {
      Name    = "development-optinist-asg-instance"
      Type    = "ECS-ASG"
      Service = "autoscaling"
    }
  }
...
resource "aws_autoscaling_group" "main" {
  ...
  tag {
    key                 = "Name"
    value               = "development-optinist-asg-instance"
  ...
  initial_lifecycle_hook {
    name                 = "development-optinist-launch-hook"
  ...
  initial_lifecycle_hook {
    name                 = "development-optinist-terminate-hook"
    default_result       = "CONTINUE"
    heartbeat_timeout    = 300
...
resource "aws_autoscaling_policy" "scale_up" {
  name                   = "development-optinist-scale-up"
...
resource "aws_autoscaling_policy" "scale_down" {
  name                   = "development-optinist-scale-down"
  scaling_adjustment     = -1
  adjustment_type        = "ChangeInCapacity"
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.main.name
...
resource "aws_ecs_service" "premium" {
  name                               = "development-premium-optinist-cloud-service"
...
resource "aws_launch_template" "premium" {
  name_prefix   = "development-optinist-premium-"
  image_id      = data.aws_ami.ecs_optimized.id
  instance_type = "t3.large"
  key_name      = aws_key_pair.development_optinist_cloud_key_pair.key_name
  ...
  tag_specifications {
    resource_type = "instance"
    tags = {
      Name    = "development-optinist-premium-instance"
      Type    = "ECS-Premium"
      Tier    = "premium"
...
# SSL/TLS
# =======
# Test environment: No custom domain, Route53, or ACM certificate.
# Access via ALB DNS directly over HTTP.

# ===========
# ECS Cluster
# ===========
resource "aws_ecs_cluster" "main" {
  name = "development-optinist-cloud-cluster"
...
resource "aws_service_discovery_private_dns_namespace" "main" {
  name = "development.optinist.local"
  vpc  = aws_vpc.main.id
}

# ECS Capacity Provider
resource "aws_ecs_capacity_provider" "main" {
  name = "development-optinist-capacity-provider"
...
resource "aws_instance" "premium" {
  ...
  tags = {
    Name          = "development-premium-${count.index + 1}"
...
resource "aws_ecs_task_definition" "autoscaling" {
  family                   = "development-optinist-cloud-taskdef"
  ...
  container_definitions = jsonencode([
    {
      name              = "development-optinist-cloud-container"
      ...
      portMappings = [
        {
          name          = "development-optinist-cloud-container-port-8000"
          containerPort = 8000
          hostPort      = 8000
          protocol      = "tcp"
        }
      ...
      environment = [
        {
          ...
          value = "/ecs/development-optinist-cloud-taskdef"
        },
        {
          name  = "PYTHONPATH"
          value = "/app/"
      ...
      mountPoints = [
        {
          sourceVolume  = "development-optinist-cloud-snmk-volume"
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"             = "/ecs/development-optinist-cloud-taskdef"
          "mode"                      = "non-blocking"
  ...
  volume {
    name = "development-optinist-cloud-snmk-volume"
  ...
  tags = {
    Name = "development-optinist-cloud-taskdef"
  }
}
...
resource "aws_ecs_task_definition" "premium" {
  family                   = "development-premium-optinist-cloud-taskdef"
  requires_compatibilities = ["EC2"]
  network_mode             = "bridge"
  cpu                      = 2048
  memory                   = 7168
  ...
  container_definitions = jsonencode([
    {
      name              = "development-premium-optinist-cloud-container"
      image             = "${var.ecr_repository_url}:latest"
      cpu               = 1536
      memory            = 6656
      memoryReservation = 4096
      ...
      portMappings = [
        {
          name          = "development-premium-optinist-cloud-container-port-8000"
          containerPort = 8000
          hostPort      = 8000
          protocol      = "tcp"
        }
      ...
      environment = [
        {
          name  = "CLOUDWATCH_LOG_GROUP"
          value = "/ecs/development-premium-optinist-cloud-taskdef"
        },
        {
          name  = "PYTHONPATH"
          value = "/app/"
      ...
      mountPoints = [
        {
          sourceVolume  = "development-premium-optinist-cloud-snmk-volume"
          containerPath = "/app/.snakemake"
          readOnly      = false
        }
      ]
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"             = "/ecs/development-premium-optinist-cloud-taskdef"
          "awslogs-multiline-pattern" = "^\\[\\d{4}-\\d{2}-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}"
          "max-buffer-size"           = "25m"
          "awslogs-region"            = "ap-northeast-1"
          "awslogs-create-group"      = "true"
  ...
  volume {
    name = "development-premium-optinist-cloud-snmk-volume"
    efs_volume_configuration {
      file_system_id     = aws_efs_file_system.snmk.id
      root_directory     = "/"
      transit_encryption = "ENABLED"
  ...
  tags = {
    Name = "development-premium-optinist-cloud-taskdef"
    Tier = "premium"
  }
}
...
resource "aws_ecs_service" "autoscaling" {
  name                               = "development-optinist-cloud-service"
  cluster                            = aws_ecs_cluster.main.id
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.autoscaling.arn
    container_name   = "development-optinist-cloud-container"
    container_port   = 8000
  }
...
#   name               = "development-optinist-ecs-cpu-scaling"
...
#   name               = "development-optinist-ecs-memory-scaling"
#   policy_type        = "TargetTrackingScaling"
#   resource_id        = aws_appautoscaling_target.autoscaling_ecs.resource_id
#   scalable_dimension = aws_appautoscaling_target.autoscaling_ecs.scalable_dimension
#   service_namespace  = aws_appautoscaling_target.autoscaling_ecs.service_namespace
