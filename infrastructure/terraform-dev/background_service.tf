...
resource "aws_launch_template" "background" {
  name_prefix   = "development-optinist-background-"
  ...
  tag_specifications {
    ...
    tags = {
      Name    = "development-optinist-background-instance"
      Type    = "ECS-Background"
      Tier    = "background"
...
resource "aws_instance" "background" {
  ...
  tags = {
    Name    = "development-optinist-background"
...
resource "aws_ecs_task_definition" "background" {
  family                   = "development-background-optinist-cloud-taskdef"
  ...
  container_definitions = jsonencode([
    {
      name              = "development-background-optinist-cloud-container"
      ...
      environment = [
        {
          ...
          value = "/ecs/development-background-optinist-cloud-taskdef"
        },
        {
          name  = "PYTHONPATH"
          value = "/app/"
      ...
      mountPoints = [
        {
          sourceVolume  = "development-background-optinist-cloud-snmk-volume"
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"             = "/ecs/development-background-optinist-cloud-taskdef"
  ...
  volume {
    name = "development-background-optinist-cloud-snmk-volume"
  ...
  tags = {
    Name = "development-background-optinist-cloud-taskdef"
    Tier = "background"
  }
}
...
resource "aws_ecs_service" "background" {
  name                               = "development-background-optinist-cloud-service"
...
resource "aws_cloudwatch_log_group" "background_logs" {
  name              = "/ecs/development-background-optinist-cloud-taskdef"
...
resource "aws_cloudwatch_metric_alarm" "background_task_stopped" {
  alarm_name          = "development-background-task-stopped"
...
resource "aws_cloudwatch_metric_alarm" "background_cpu_high" {
  alarm_name          = "development-background-cpu-high"
...
resource "aws_cloudwatch_metric_alarm" "background_memory_high" {
  alarm_name          = "development-background-memory-high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "3"
