...
variable "test_users" {
  description = "Development user configuration with Firebase UIDs and subscription details"
  type = list(object({
    email                = string
    name                 = string
    firebase_uid         = string
...
output "ssh_key_name" {
  ...
  value       = aws_key_pair.development_optinist_cloud_key_pair.key_name
}
...
output "optinist_cloud_user_access_key_id" {
  description = "Access Key ID for development-optinist-cloud-user"
  value       = aws_iam_access_key.development_optinist_cloud_user_access_key.id
}

output "optinist_cloud_user_secret_access_key" {
  description = "Secret Access Key for development-optinist-cloud-user"
  value       = aws_iam_access_key.development_optinist_cloud_user_access_key.secret
  sensitive   = true
}
...
output "test_users" {
  description = "Development user configuration for load testing (includes Firebase UIDs)"
  value       = var.test_users
  sensitive   = true
}

# Domain outputs (test env: no custom domain)
...
output "alb_listener_https_arn" {
  description = "ARN of the ALB listener used for premium routing rules"
  value       = aws_lb_listener.autoscaling_https.arn
}
