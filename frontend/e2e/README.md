...
`16-storage-aws`'s `S3-21..23`, because they spend premium capacity exactly as
`15-premium-aws` does and a single variable is what makes "may I assign a real
instance?" one decision rather than two. Those rows carry their own IDs rather
...
| `16-storage-aws`   | S3-01..05, S3-21..23 | real-S3 truth for the storage rows: the per-user bucket and an uploaded object read back from S3 rather than from the API, which swallows S3 failures; the merged listing and sync round-trip; sample import and workspace delete against the real prefixes; an anonymous public read of a freshly published run; the public sync-error state and its Retry recovery; and the publish-time repair of a broken local config with a five-record batch draining in one sync run. `@slow` plus `RUN_S3_AWS=1`. Rows 01..03 are registered once per tier from one body — the sheets ask the same S3 question of the free and the premium account, and the only differences are which account signs in, which table carries `active_workflow_count`, and whether the helper calls need the premium routing headers — but they keep separate IDs, `S3-01..03` free and `S3-21..23` premium (the 20s block, paired on the last digit), so a run that omitted `RUN_PREMIUM_AWS` cannot tick the premium row off the free variant's pass. `S3-21..23` additionally need `RUN_PREMIUM_AWS=1`: they hold a real assignment for the run and release it in an `afterEach`, and they skip with a reason (rather than fail) when the dev pool has no capacity to place or cannot keep a premium task serving. S3-04 and S3-05 stay free-only, each saying why in the spec |
