...
| `TEST_LIFECYCLE_EMAIL` / `TEST_LIFECYCLE_PASSWORD` | optional                          | enables `11-lifecycle` (LC-01..05, LC-07..10, LC-14..15, LC-17..20, LC-24..31; local stack only) and `23-subscription-lifecycle` (LC-06, LC-11..13, LC-16, LC-21..23; local or deployed). Both rewrite this account's plan, expiry and usage, and LC-16 deletes a throwaway it registers itself - use a dedicated address, never a shared account. `11-lifecycle` registers and verifies the account itself on first run; against a deployed environment that is not possible (forcing `email_verified` needs the Admin SDK), so create it once and verify it out of band |
