...
| `TEST_FREE_DOWNGRADE` / `TEST_FREE_DOWNGRADE_PASSWORD` | deployed `11-lifecycle` only | a real formerly-paid account carrying more than the free limit. LC-08 drops its premium row so `determine_lifecycle` reports FREE - a genuine downgrade rather than a faked one - then restores plan and quota |
...
- **`11-lifecycle` rows must not claim a real premium instance.** A login by an
  ...
  holds pool capacity until the 2h inactivity sweep reclaims it.
