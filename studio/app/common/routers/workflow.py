...
async def import_sample_data(
    ...
    # ------------------------------------------------------------
    # Copy sample data to EFS for batch execution (when available)
    # Only needed when using EFS storage mode (not S3 storage mode)
    # ------------------------------------------------------------

    # Check if we should also copy to EFS for batch execution
    # Only copy when NOT using S3 (i.e., using EFS storage mode as fallback)
    if not RemoteStorageController.is_available():
        # EFS storage mode - need to copy files to EFS mount for batch access
        ...
        logger.info(
            "Using S3 storage mode - skipping EFS copy "
            "(batch jobs will read directly from S3)"
        )
