...
class JobMonitoringThread(threading.Thread):
    ...
    def run(self):
        ...
        print(
            f"BATCH: JobMonitoringThread.run() "
        ...
        if self.client_id is not None:
            ...
            print(f"BATCH: Set client_id context to {self.client_id}", flush=True)
        else:
            print(
                "BATCH: WARNING - No client_id set for monitoring thread!",
                flush=True,
            )

        logger.info("Job monitoring thread running")
        print("BATCH: After logger.info in monitoring thread", flush=True)
...
class BatchUtils:
    ...
    def __init__(self, workspace_id: str, unique_id: str):
        print(
            f"BATCH: BatchUtils.__init__ ENTRY for {workspace_id}/{unique_id}",
            flush=True,
        )
        self.workspace_id = workspace_id
        self.unique_id = unique_id

        print("BATCH: Creating batch_client...", flush=True)
        self.batch_client = boto3.client(
            "batch", region_name=BATCH_CONFIG.AWS_DEFAULT_REGION
        )
        print("BATCH: batch_client created successfully", flush=True)

        print("BATCH: Creating ecr_client...", flush=True)
        self.ecr_client = boto3.client(
            "ecr", region_name=BATCH_CONFIG.AWS_DEFAULT_REGION
        )
        print("BATCH: ecr_client created successfully", flush=True)

        print("BATCH: Creating s3_client...", flush=True)
        self.s3_client = boto3.client("s3", region_name=BATCH_CONFIG.AWS_DEFAULT_REGION)
        print("BATCH: s3_client created successfully", flush=True)

        print("BATCH: Creating logs_client...", flush=True)
        self.logs_client = boto3.client(
            "logs", region_name=BATCH_CONFIG.AWS_DEFAULT_REGION
        )
        print("BATCH: logs_client created successfully", flush=True)

        # Get current user info for subscription plan-based queue selection
        print("BATCH: Calling _get_current_user()...", flush=True)
        self.current_user = self._get_current_user()
        print(f"BATCH: _get_current_user() returned: {self.current_user}", flush=True)
        ...
        print("BATCH: About to call get_job_queue_for_user()...", flush=True)
        try:
            job_queue_for_monitoring = self.get_job_queue_for_user()
            print(
                f"BATCH: get_job_queue_for_user() returned: "
            ...
            print(
                f"BATCH: get_job_queue_for_user() raised exception: {e}",
                flush=True,
            )
        ...
        print("BATCH: About to get client_id...", flush=True)
        current_client_id = AppLogger.get_client_id()
        print(f"BATCH: Got client_id={current_client_id}", flush=True)
        print(
            f"BATCH: Creating JobMonitoringThread with client_id="
            f"{current_client_id}, queue={job_queue_for_monitoring}",
            flush=True,
        )

        print("BATCH: About to create JobMonitoringThread...", flush=True)
        ...
        print("BATCH: JobMonitoringThread instance created", flush=True)
        self.monitoring_enabled = True
        print(
            f"BATCH: JobMonitoringThread created, "
            f"monitoring_enabled={self.monitoring_enabled}",
            flush=True,
        )
        print(
            f"BATCH: BatchUtils.__init__ COMPLETE for {workspace_id}/{unique_id}",
            flush=True,
        )
