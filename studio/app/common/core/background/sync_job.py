...
class PublishedExperimentSyncJob:
    ...
    async def _sync_thumbnails(cls):
        ...
        async def sync_thumbnails_for_experiment(workspace_id, unique_id, exp_id):
            async with semaphore:
                try:
                    ...
                    logger.warning(
                        f"Failed to sync thumbnails for {workspace_id}/{unique_id}: {e}"
                    )
                    return (workspace_id, unique_id, exp_id, False)

        tasks = [sync_thumbnails_for_experiment(w, u, e) for w, u, e in pending]
    ...
    @classmethod
    def _get_pending_experiments(cls, limit: int = None) -> List[Tuple[str, str, int]]:
        """
