...
def get_user_subscription_tier(user_id: int = None) -> str:
    """
    Get user subscription tier ('free' or 'paid').

    Args:
        user_id: User ID. If None, attempts to get current user from context.

    Returns:
        str: User subscription tier ('free' or 'paid')
    """
    try:
        if user_id is None:
            ...
            if user_context:
                return user_context.get("subscription_tier", "free")
            else:
                logger.debug("No user context available, defaulting to free tier")
                return "free"
        else:
            # Get subscription details for specific user
            subscription_details = get_user_subscription_details(user_id)
            if subscription_details:
                plan_name = subscription_details.get("plan_name", "Free")
                return "paid" if plan_name == "Premium" else "free"
            else:
                logger.debug(
                    f"No subscription details found for user {user_id}, "
        ...
        logger.warning(
            f"Failed to get user subscription tier: {e}, " " defaulting to free tier"
        )
        return "free"
